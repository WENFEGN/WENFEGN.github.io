...
const DB = (() => {
  ...
  async function update(table, id, patch) {
    ...
    if (idx >= 0) { all[idx] = Object.assign({}, all[idx], patch); localWrite(table, all); noteSaved(); return all[idx]; }
    return null;
  }
...
/* 自动保存状态提示：任何一次写入成功后，顶部 chip 更新为最新保存时间 */
function noteSaved() {
  const el = $('#syncStatus');
  if (!el) return;
  const t = new Date();
  const ts = String(t.getHours()).padStart(2, '0') + ':' + String(t.getMinutes()).padStart(2, '0') + ':' + String(t.getSeconds()).padStart(2, '0');
  el.textContent = DB.isCloud() ? '☁️ 已同步 ' + ts : '💾 本机已保存 ' + ts;
  el.classList.toggle('cloud', DB.isCloud());
  el.classList.remove('hidden');
}

/* 防抖：输入停顿 ms 毫秒后才执行，用于"边改边自动保存" */
...
const App = {
  ...
  registerSW() {
    ...
    // 新 SW 接管后自动刷新一次页面，保证用户立刻看到新版代码（仅刷一次，防循环）
    if (!sessionStorage.getItem('cl_sw_reloaded')) {
      let reloading = false;
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        if (reloading) return;
        reloading = true;
        sessionStorage.setItem('cl_sw_reloaded', '1');
        location.reload();
      });
    }
    navigator.serviceWorker.register('sw.js').then(reg => {
      // 若有等待中的新 SW，主动通知它立即接管
  ...
  async orderRecipe(id) {
    ...
    if (!r) return;
    const who = myPerson();
    const whoName = (who === 'male' ? '👦 ' : '👧 ') + pName(who);
    openModal(`
      <h3>🍽️ 确认点菜</h3>
      <p style="text-align:center;margin-bottom:14px;font-size:15px;">确认要点「<b>${esc(r.name)}</b>」吗？<br>
      <small style="color:var(--text-light);">将以 <b>${esc(whoName)}</b> 的身份下单 · 对方打开应用后将看到红点提醒</small></p>
        ...
        html += orders.map(o => {
          ...
          return `<div class="order-item">
            <div style="flex:1">
              ...
              <div style="font-size:11px;color:var(--text-light);">${esc(personTag(o.ordered_by))} · ${esc(o.order_date)}</div>
            </div>
  ...
  renderRestrictionBanner() {
    ...
    if (r && (r.male_restrictions || r.female_restrictions)) {
      let parts = [];
      if (r.male_restrictions) parts.push(personTag('male') + '忌口：' + r.male_restrictions);
      if (r.female_restrictions) parts.push(personTag('female') + '忌口：' + r.female_restrictions);
  ...
  renderRecipeRank() {
    ...
    let tagHtml = '';
    if (topFemale) tagHtml += `<span class="rank-tag pink">${esc(personTag('female'))} 最爱点：${esc(topFemale[0])} ×${topFemale[1]}</span>`;
    if (topMale) tagHtml += `<span class="rank-tag gold">${esc(personTag('male'))} 最爱点：${esc(topMale[0])} ×${topMale[1]}</span>`;
  ...
  renderHealth() {
    ['male', 'female'].forEach(person => {
      ...
      // 最新一条身高体重（跳过纯健身打卡记录）
      let latest = null;
      for (let i = rows.length - 1; i >= 0; i--) { if (rows[i].weight != null) { latest = rows[i]; break; } }
      const height = latest ? latest.height : null;
      const weight = latest ? latest.weight : null;
      const nameEl = $(`#healthName${cap(person)}`);
      if (nameEl) nameEl.textContent = personTag(person);
      ...
      // 健身打卡状态（只统计 sport=健身 的打卡记录）
      const monthKey = todayStr().slice(0, 7);
      const monthCnt = rows.filter(h => h.sport === '健身' && h.record_date.startsWith(monthKey)).length;
      const doneToday = rows.some(h => h.sport === '健身' && h.record_date === todayStr());
      const wEl = $(`#workout${cap(person)}`);
      if (wEl) {
        wEl.textContent = `💪 本月 ${monthCnt} 次 · 今日${doneToday ? '已打卡 ✅' : '未打卡'}`;
        wEl.className = 'workout-today' + (doneToday ? ' done' : '');
      }
    });
  ...
  renderWeightChart() {
    const dates = [...new Set(this.state.health.filter(h => h.weight != null).map(h => h.record_date))].sort();
    const maleData = dates.map(d => { const r = this.state.health.find(h => h.record_date === d && h.person === 'male' && h.weight != null); return r ? r.weight : null; });
    const femaleData = dates.map(d => { const r = this.state.health.find(h => h.record_date === d && h.person === 'female' && h.weight != null); return r ? r.weight : null; });
    this.renderLineChart('weightChart', 'weightChart', dates, [
      { label: personTag('male') + '体重', data: maleData, color: '#4dabf7' },
      { label: personTag('female') + '体重', data: femaleData, color: '#f783ac' }
    ]);
  },

  /* ── 身高体重录入（与健身打卡分开） ── */
  openBodyEdit(person) {
    const rows = this.state.health.filter(h => h.person === person && h.weight != null);
    const latest = rows[rows.length - 1];
    openModal(`
      <h3>${personTag(person)} · 身高体重</h3>
      <div class="form-group">
        <label>身高（cm，仅首次填一次）</label>
        <input type="number" id="beHeight" placeholder="如 175" value="${latest && latest.height ? latest.height : ''}">
      </div>
      <div class="form-group">
        <label>今日体重（kg）</label>
        <input type="number" id="beWeight" placeholder="如 65.5" step="0.1" value="${latest ? latest.weight : ''}">
      </div>
      <p style="font-size:11px;color:var(--text-light);">💪 健身打卡用卡片上的按钮一键记录，与身高体重分开。</p>
      <div class="modal-actions">
        <button class="btn btn-outline" onclick="closeModal()">取消</button>
        <button class="btn btn-primary" onclick="App.saveBody('${person}')">保存</button>
      </div>`);
  },

  async saveBody(person) {
    const height = parseFloat($('#beHeight').value) || null;
    const weight = parseFloat($('#beWeight').value);
    if (!weight) { toast('请填写体重'); return; }
    // 当天已有身高体重记录则更新，否则新增（避免重复行）
    const row = this.state.health.find(h => h.person === person && h.record_date === todayStr() && h.weight != null);
    if (row) await DB.update('health_data', row.id, { height, weight });
    else await DB.insert('health_data', { person, height, weight, record_date: todayStr() });
    closeModal();
    toast('✅ 身高体重已保存');
    this.state.health = await DB.select('health_data', { order: ['record_date', true] });
    this.renderHealth();
  ...
  /* ── 健身打卡：只记"今天练没练"，一键切换 ── */
  async toggleWorkout(person) {
    const row = this.state.health.find(h => h.person === person && h.record_date === todayStr() && h.sport === '健身');
    if (row) {
      await DB.remove('health_data', row.id);
      toast(personTag(person) + ' 已取消今日健身打卡');
    } else {
      await DB.insert('health_data', { person, sport: '健身', record_date: todayStr() });
      toast('💪 ' + personTag(person) + ' 今日健身打卡成功！');
    }
    this.state.health = await DB.select('health_data', { order: ['record_date', true] });
    this.renderHealth();
  },
  ...
  /* ── 健身统计：按月 / 按年 ── */
  setWorkoutView(view) {
    this.state.workoutView = view;
    const m = $('#workoutViewMonth'), y = $('#workoutViewYear');
    if (m) m.classList.toggle('active', view === 'month');
    if (y) y.classList.toggle('active', view === 'year');
    this.renderWorkoutStats();
  },

  countWorkout(person, prefix) {
    return this.state.health.filter(h => h.person === person && h.sport === '健身' && h.record_date.startsWith(prefix)).length;
  },

  renderWorkoutStats() {
    const view = this.state.workoutView || 'month';
    const canvas = $('#workoutChart');
    if (!canvas) return;
    // 清掉之前可能残留的空状态提示
    const prevEmpty = canvas.parentElement.querySelector('.chart-empty');
    if (prevEmpty) prevEmpty.remove();
    let labels, maleData, femaleData;
    if (view === 'month') {
      labels = []; maleData = []; femaleData = [];
      const now = new Date();
      for (let i = 11; i >= 0; i--) {
        const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
        labels.push(`${d.getMonth() + 1}月`);
        maleData.push(this.countWorkout('male', key));
        femaleData.push(this.countWorkout('female', key));
      }
    } else {
      const years = [...new Set(this.state.health.filter(h => h.sport === '健身').map(h => h.record_date.slice(0, 4)))].sort();
      const curYear = String(new Date().getFullYear());
      if (!years.includes(curYear)) years.push(curYear);
      labels = years.map(y => y + '年');
      maleData = years.map(y => this.countWorkout('male', y));
      femaleData = years.map(y => this.countWorkout('female', y));
    }
    if (!maleData.some(v => v > 0) && !femaleData.some(v => v > 0)) {
      this.destroyChart('workoutChart');
      const div = document.createElement('div');
      div.className = 'empty-state chart-empty';
      div.style.padding = '16px';
      div.textContent = '还没有健身打卡记录，点卡片上的"💪 健身打卡"一键记录吧';
      canvas.parentElement.appendChild(div);
      return;
    }
    this.renderBarChart('workoutChart', 'workoutChart', labels, [
      { label: personTag('male') + ' 健身', data: maleData, color: '#4dabf7' },
      { label: personTag('female') + ' 健身', data: femaleData, color: '#f783ac' }
    ]);
  },
  ...
  renderWorkCalendar() {
    ...
    for (let d = 1; d <= daysInMonth; d++) {
      ...
      const dots = scheds.map(s => {
        ...
        return `<span class="sched-dot" style="background:${cfg.color}" title="${esc(personTag(s.person))} ${cfg.name}"></span>`;
      }).join('');
  ...
  openSchedModal(date) {
    ...
    if (existing.length) {
      html += existing.map(s => {
        ...
        return `<div style="display:flex;justify-content:space-between;align-items:center;padding:8px;background:var(--primary-light);border-radius:8px;margin-bottom:6px;">
          <span>${esc(personTag(s.person))} · <span class="sched-badge" style="background:${cfg.color}">${cfg.name}</span></span>
    ...
    html += `<div class="form-group"><label>人员</label>
        <select id="scPerson"><option value="male" ${my === 'male' ? 'selected' : ''}>👦 ${esc(pName('male'))}</option><option value="female" ${my === 'female' ? 'selected' : ''}>👧 ${esc(pName('female'))}</option></select></div>
  ...
  renderScheduleList() {
    ...
    wrap.innerHTML = list.map(s => {
      ...
      return `<div class="schedule-item">
        ...
        <span class="sched-info">${esc(personTag(s.person))}</span>
  ...
  renderTxList() {
    ...
    list.innerHTML = txs.map(t => {
      ...
      return `<div class="tx-item">
        ...
        <div class="tx-body">
          <div class="tx-cat">${esc(t.category)} <small style="color:var(--text-light)">· ${esc(t.payer === 'male' || t.payer === 'female' ? personTag(t.payer) : '🤝 共同')}</small></div>
  ...
  openTxModal() {
    ...
    openModal(`
      ...
      <div class="form-row">
        <div class="form-group"><label>类型</label>
          <select id="txType" onchange="App.onTxTypeChange()"><option value="expense">支出</option><option value="income">收入</option></select></div>
      ...
      <div class="form-row">
        <div class="form-group"><label>类别</label>
          <select id="txCategory">${CONFIG.EXPENSE_CATEGORIES.map(c => `<option>${c}</option>`).join('')}</select></div>
        <div class="form-group"><label>支付人</label>
          <select id="txPayer"><option value="male" ${my === 'male' ? 'selected' : ''}>👦 ${esc(pName('male'))}</option><option value="female" ${my === 'female' ? 'selected' : ''}>👧 ${esc(pName('female'))}</option><option value="both">🤝 共同</option></select></div>
      </div>
  ...
  onTxTypeChange() {
    ...
    const cats = t === 'income' ? CONFIG.INCOME_CATEGORIES : CONFIG.EXPENSE_CATEGORIES;
  ...
  setLedgerView(view) {
    this.state.ledgerView = view;
    const m = $('#ledgerViewMonth'), y = $('#ledgerViewYear');
    if (m) m.classList.toggle('active', view === 'month');
    if (y) y.classList.toggle('active', view === 'year');
    this.renderLedgerCharts();
  },

  renderLedgerCharts() {
    const view = this.state.ledgerView || 'month';
    const now = new Date();
    const txs = this.state.txs;
    // 收支统计：按月（近12个月）/ 按年
    let labels, expData, incData;
    if (view === 'month') {
      labels = []; expData = []; incData = [];
      for (let i = 11; i >= 0; i--) {
        const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
        const key = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
        labels.push(`${d.getMonth() + 1}月`);
        expData.push(txs.filter(t => t.tx_date.startsWith(key) && t.type === 'expense').reduce((s, t) => s + Number(t.amount), 0));
        incData.push(txs.filter(t => t.tx_date.startsWith(key) && t.type === 'income').reduce((s, t) => s + Number(t.amount), 0));
      }
    } else {
      const years = [...new Set(txs.map(t => t.tx_date.slice(0, 4)))].sort();
      if (!years.length) years.push(String(now.getFullYear()));
      labels = years.map(y => y + '年');
      expData = years.map(y => txs.filter(t => t.tx_date.startsWith(y) && t.type === 'expense').reduce((s, t) => s + Number(t.amount), 0));
      incData = years.map(y => txs.filter(t => t.tx_date.startsWith(y) && t.type === 'income').reduce((s, t) => s + Number(t.amount), 0));
    }
    this.renderBarChart('monthlyChart', 'monthlyChart', labels, [
    ...
    txs.filter(t => t.type === 'income').forEach(t => {
      ...
      monthMap[k] = (monthMap[k] || 0) + Number(t.amount);
    });
    const keys = Object.keys(monthMap).sort();
    if (keys.length) {
      let cum = 0;
      const cumLabels = keys.map(k => k.slice(5) + '月');
      const cumData = keys.map(k => { cum += monthMap[k]; return Math.round(cum * 100) / 100; });
      this.renderLineChart('cumIncomeChart', 'cumIncomeChart', cumLabels, [
        { label: '累计总收入', data: cumData, color: '#40c057' }
      ]);
    } else {
      this.destroyChart('cumIncomeChart');
      this.showChartEmpty('cumIncomeChart', '还没有收入记录，记一笔收入后这里会出现累计曲线');
    }

    // 本月支出 / 收入分类占比
    const nowKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
    this.renderCategoryPie('categoryChart', 'categoryChart', txs.filter(t => t.tx_date.startsWith(nowKey) && t.type === 'expense'), '本月暂无支出数据');
    this.renderCategoryPie('incomeCategoryChart', 'incomeCategoryChart', txs.filter(t => t.tx_date.startsWith(nowKey) && t.type === 'income'), '本月暂无收入数据');
  },

  renderCategoryPie(canvasId, key, rows, emptyMsg) {
    const catCount = {};
    rows.forEach(t => { catCount[t.category] = (catCount[t.category] || 0) + Number(t.amount); });
    const entries = Object.entries(catCount).sort((a, b) => b[1] - a[1]);
    if (entries.length) {
      this.renderPieChart(canvasId, key, entries.map(e => e[0]), entries.map(e => e[1]));
    } else {
      this.destroyChart(key);
      this.showChartEmpty(canvasId, emptyMsg);
    }
  },
  ...
  showChartEmpty(canvasId, msg) {
    ...
    box.querySelectorAll('.chart-empty').forEach(el => el.remove());
    const div = document.createElement('div');
    div.className = 'empty-state chart-empty';
    div.style.padding = '16px';
    div.textContent = msg;
  ...
  async uploadImage(file, folder) {
    ...
    if (DB.isCloud() && window.supabase) {
      try {
        ...
        if (error) throw new Error(errMsg(error, 'storage'));
  ...
  openProfile() {
    ...
    openModal(`
      ...
      <div class="settings-group">
        <h4>👤 我的身份与昵称</h4>
        <div class="form-group"><label>我是</label>
          <select id="cfgPerson" onchange="App.saveIdentity()">
            <option value="male" ${my === 'male' ? 'selected' : ''}>👦 ${esc(pName('male'))}</option>
            <option value="female" ${my === 'female' ? 'selected' : ''}>👧 ${esc(pName('female'))}</option>
          </select>
          <small style="color:var(--text-light);">点菜 / 接单 / 记账 / 排班会自动按你的身份记录</small></div>
        <div class="form-group"><label>👦 男方昵称</label><input id="nickMale" placeholder="如：小陈" value="${esc(getNick('male'))}"></div>
        <div class="form-group"><label>👧 女方昵称</label><input id="nickFemale" placeholder="如：小美" value="${esc(getNick('female'))}"></div>
        <p style="font-size:11px;color:var(--text-light);">身份与昵称仅存本机，修改后自动保存。昵称会显示在食堂、身心、搬砖、账本等处。</p>
      </div>
      <div class="settings-group">
        <h4>🚫 忌口设置（显示在食堂顶部横幅）</h4>
        <div class="form-group"><label>${esc(personTag('male'))}忌口</label><input id="rMale" placeholder="如：不吃香菜、海鲜过敏" value="${maleR && maleR.male_restrictions ? esc(maleR.male_restrictions) : ''}"></div>
        <div class="form-group"><label>${esc(personTag('female'))}忌口</label><input id="rFemale" placeholder="如：不吃辣" value="${maleR && maleR.female_restrictions ? esc(maleR.female_restrictions) : ''}"></div>
    ...
    this.bindProfileAutoSave();
  },

  /* 设置页自动保存：昵称 / 忌口 / 纪念日，输入停顿即存 */
  bindProfileAutoSave() {
    const saveNick = (person) => debounce(() => {
      const v = (document.getElementById('nick' + cap(person)) || {}).value;
      if (v == null) return;
      localStorage.setItem('cl_nick_' + person, v.trim());
      noteSaved();
      toast(personTag(person) + ' 昵称已自动保存');
      this.renderHealth();
      this.renderRestrictionBanner();
    }, 1200);
    const nMale = $('#nickMale'), nFemale = $('#nickFemale');
    if (nMale) nMale.addEventListener('input', saveNick('male'));
    if (nFemale) nFemale.addEventListener('input', saveNick('female'));
    const rMale = $('#rMale'), rFemale = $('#rFemale');
    const saveRest = debounce(() => this.saveRestrictions(true), 1500);
    if (rMale) rMale.addEventListener('input', saveRest);
    if (rFemale) rFemale.addEventListener('input', saveRest);
    const love = $('#loveStart');
    if (love) love.addEventListener('change', () => this.saveLoveStart());
  },

  saveIdentity() {
    const v = $('#cfgPerson').value;
    if (v !== 'male' && v !== 'female') return;
    localStorage.setItem('cl_my_person', v);
    noteSaved();
    toast('✅ 已记住：你是' + pName(v) + '，后续点菜等操作自动按此身份');
  },
  ...
  /* ── 工具 ── */
  ensureIdentity() {
    const v = localStorage.getItem('cl_my_person');
    if (v === 'male' || v === 'female') return;
    openModal(`
      <h3>👋 欢迎使用 CoupleLife</h3>
      <p style="text-align:center;font-size:14px;margin-bottom:6px;">先告诉我：你是哪一位？</p>
      <p style="text-align:center;font-size:12px;color:var(--text-light);margin-bottom:14px;">选一次就好，以后点菜 / 接单 / 记账 / 排班都会自动按你的身份记录，不用再每次选择～</p>
      <div class="modal-actions">
        <button class="btn btn-primary" onclick="App.setIdentity('male')">👦 ${esc(pName('male'))}</button>
        <button class="btn btn-primary" style="background:#f783ac;border-color:#f783ac;" onclick="App.setIdentity('female')">👧 ${esc(pName('female'))}</button>
      </div>`);
  },

  setIdentity(person) {
    if (person !== 'male' && person !== 'female') return;
    localStorage.setItem('cl_my_person', person);
    noteSaved();
    closeModal();
    toast('✅ 已记住：你是' + pName(person) + '，以后自动按这个身份记录');
  },
...
/* 身份与昵称：存本机，点菜/接单/记账/排班等自动按"我"的身份 */
function myPerson() { const v = localStorage.getItem('cl_my_person'); return (v === 'male' || v === 'female') ? v : 'male'; }
function getNick(person) { return (localStorage.getItem('cl_nick_' + person) || '').trim(); }
function pName(person) { const n = getNick(person); return n || (person === 'male' ? '男方' : '女方'); }
function personTag(person) { return (person === 'male' ? '👦 ' : '👧 ') + pName(person); }
function whoAmI() { return myPerson(); }
...
const LockGate = (() => {
  ...
  function init() {
    ...
    if (hasPwd() && isUnlocked()) {
      ...
      setTimeout(() => App.ensureIdentity(), 0);
      return;
    }
